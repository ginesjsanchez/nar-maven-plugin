 ...
 * surefire plugin to accomodate a few things for the NAR plugin:
 * <P>
...
public class NarIntegrationTestMojo extends AbstractDependencyMojo {
	 ...
	 * property maven.test.skip.exec. Use -DskipTests instead; it's shorter.
	 * 
	 * @deprecated
	 * @since 2.3
	 */
	...
	/**
	 * The base directory of the project being tested. This can be obtained in your
	 * unit test by System.getProperty("basedir").
	 * 
	 */
	...
	/**
	 * List of patterns (separated by commas) used to specify the tests that should
	 * be included in testing. When not specified and when the <code>test</code>
	...
	/**
	 * List of patterns (separated by commas) used to specify the tests that should
	 * be excluded in testing. When not specified and when the <code>test</code>
	 * parameter is not specified, the default excludes will be
	 * <code>**&#47;*$*</code> (which excludes all inner classes). This parameter is
	 * ignored if TestNG suiteXmlFiles are specified.
	 */
	@Parameter
	...
	/**
	 * List of properties for configuring all TestNG related configurations. This is
	 * the new preferred method of configuring TestNG.
	 */
	@Parameter
	...
	/**
	 * Option to print summary of test suites or just print the test cases that has
	 * errors.
	 */
	...
	// DUNS changed to nar. because of naming conflict
	/**
	 * When forking, set this to true to redirect the unit test standard output to a
	 * file (found in reportsDirectory/testName-output.txt).
	 * 
	 * @since 2.3
	 */
	...
	/**
	 * Set this to "true" to cause a failure if there are no tests to run. Defaults
	 * to false.
	 *
	 * @since 2.4
	 */
	 ...
	 * forking options. For the default, the jvm will be the same as the one used to
	 * run Maven.
	 * 
	 * @since 2.1
	 */
	 ...
	 * options specified in the argLine).
	 *
	 * @since 2.4
	 */
	 ...
	 */
	@Parameter
	private Map<Object, Object> environmentVariables = new HashMap<>();
	 ...
	 * Setting it to false helps with some problems caused by conflicts between xml
	 * parsers in the classpath and the Java 5 provider parser.
	 * 
	 * @since 2.1
	 */
	 ...
	 * one of the groups specified here will be included in test run, if specified.
	...
	/**
	 * (TestNG only) Excluded groups. Any methods/classes/etc with one of the groups
	 * specified in this list will specifically not be run. This parameter is
	 * overridden if suiteXmlFiles are specified.
	 *
	 * @since 2.2
	 */
	 ...
	 * Note that suiteXmlFiles is incompatible with several other parameters on this
	 * plugin, like includes/excludes. This parameter is ignored if the "test"
	 * parameter is specified (allowing you to run a single test instead of an
	 * entire suite).
	 * 
	 * @since 2.2
	 */
	 ...
	 * threads should be allocated for this execution. Only makes sense to use in
	...
	/**
	 * (TestNG only) When you use the parallel attribute, TestNG will try to run all
	 * your test methods in separate threads, except for methods that depend on each
	 * other, which will be run in the same thread in order to respect their order
	 * of execution.
	 * 
	 ...
	 * loading.html for a more detailed explanation of manifest-only jars and their
	 * benefits.) Default value is "true". Beware, setting this to "false" may cause
	 ...
	 * cases. To disable the assertions, set this flag to <code>false</code>.
	 * 
	 * @since 2.3.1
	 */
	...
	private void addReporters(final SurefireBooter surefireBooter, final boolean forking) {
		...
		if (this.useFile) {
			...
			if (BRIEF_REPORT_FORMAT.equals(this.reportFormat)) {
				surefireBooter.addReport(BriefFileReporter.class.getName(),
						new Object[] { this.reportsDirectory, trimStackTrace });
		...
		if (!this.disableXmlReport) {
			surefireBooter.addReport(XMLReporter.class.getName(),
					new Object[] { this.reportsDirectory, trimStackTrace });
		}
	}
	...
	private SurefireBooter constructSurefireBooter() throws MojoExecutionException, MojoFailureException {
		...
		if (tc != null) {
			...
			if (ForkMode.NEVER.equals(this.forkMode)) {
				this.forkMode = ForkMode.ONCE.getName();
			}
			if (this.jvm != null) {
		...
		if (this.additionalClasspathElements != null) {
			for (final String classpathElement : this.additionalClasspathElements) {
				getLog().debug("  " + classpathElement);
				surefireBooter.addClassPathUrl(classpathElement);
			}
		}
		...
		// Forking
		// ----------------------------------------------------------------------
//		Platform pluginPlatform = new Platform();
//		ForkMode effectiveforkMode = ForkMode.get(this.forkMode);
//		final ForkConfiguration fork = new ModularClasspathForkConfiguration(classpath, getOutputDirectory(),
//				debugForkedProcess, workingDirectory, properties, argLine, getPluginContext(), null, false,
//				(effectiveforkMode.getForkCount() >= 0 ? effectiveforkMode.getForkCount() : 4),
//				effectiveforkMode.isReuseForks(), new Platform(), new PluginConsoleLogger(logger),
//				new SurefireForkNodeFactory());
...
////        @Nonnull ForkNodeFactory forkNodeFactory) {
		...
		if (fork.isForking()) {
			...
			final List<NarArtifact> dependencies = getNarArtifacts(); // TODO: get seems heavy, not
				...
				for (final String binding2 : bindings) {
					...
					if (!binding.equals(Library.STATIC)) {
						final File depLibPathEntry = getLayout().getLibDirectory(getUnpackDirectory(),
								dependency.getArtifactId(), dependency.getBaseVersion(), getAOL().toString(), binding);
			...
			if (this.argLine != null) {
				final List<String> args = Arrays.asList(this.argLine.split(" "));
	 ...
	 * configuration method. (if any are defined the old way)
	 */
	 ...
	 * dependencies are used to declare the paths of shared and jni libraries in
	 * java.library.path
	 */
	@Override
	...
	private void showMap(final Map<Object, Object> map, final String setting) {
	...
	private boolean verifyParameters() throws MojoFailureException {
		...
		if (this.useSystemClassLoader != null && ForkMode.NEVER.equals(this.forkMode)) {
