	...
	public final void addIncludePaths(final CCTask task, final String outType)
		...
		if (this.include) {
			if (this.includePaths != null) {
				for (final String path : this.includePaths) {
					task.createIncludePath()
							.setPath(new File(this.mojo.getJavaHome(this.mojo.getAOL()), path).getPath());
				}
			} else {
				final String prefix = this.mojo.getAOL().getKey() + ".java.";
				final String includes = NarProperties.getInstance(this.mojo.getMavenProject())
						.getProperty(prefix + "include");
				if (includes != null) {
					final String[] path = includes.split(";");
					for (final String element : path) {
						task.createIncludePath()
								.setPath(new File(this.mojo.getJavaHome(this.mojo.getAOL()), element).getPath());
					}
				}
			}
		}
		...
		if (this.link) {
			if (os.equals(OS.MACOSX)) {
				...
				if (this.runtimeDirectory == null) {
					this.runtimeDirectory = NarProperties.getInstance(this.mojo.getMavenProject())
							.getProperty(prefix + "runtimeDirectory");
					if (this.runtimeDirectory == null) {
